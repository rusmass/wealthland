 ...
 */
@Component("roleLoanInfoHandler")
public class RoleLoanInfoHandler {

    private Logger logger = Logger.getLogger(RoleLoanInfoHandler.class);
...
//        roleLoanInfo.setCreditLoan(0);
    ...
    private void outerLoan(RoleInfo roleInfo, int bankMoney, int creditMoney) {
        ...
        //更新角色的金币
        roleDataManageInfoHandler.updateRoleCash(roleDataManageInfo, totalMoney);
        ...
        if (bankMoney > 0) {
            ...
            if (bankMoney <= roleLoanInfo.getBankCanLoan()) {
                ...
                //增加负债
                roleDebtsInfoHandler.addNewDebt(roleInfo, bankLoanDept);

                //新建支出
                SpendInfo bankLoanSpend = new SpendInfo(bankLoanName, -bankLoanInterest);

                //增加支出
                roleSpendInfoHandler.addNewSpend(roleInfo, bankLoanSpend);
        ...
        if (creditMoney > 0) {
            ...
            if (creditMoney <= roleLoanInfo.getCreditCanLoan()) {
                ...
                //增加支出
                roleSpendInfoHandler.addNewSpend(roleInfo, creditLoanSpend);
        ...
        roleLoanRecordInfoHandler.addLoanRecord(roleLoanRecordInfo, loanRecord);
    }
    ...
    public int repay(RoleInfo roleInfo, int repayType, String repayName, int outOrIn) {
        ...
        if (repayType == 0) {
            ...
            if (basicDebt.containsKey(repayName)) {
                ...
                //更新玩家金币
                roleDataManageInfoHandler.updateRoleCash(roleDataManageInfo, -money);

                //删除此负债
                roleDebtsInfoHandler.removeBasicDebt(roleDebtsInfo, repayName);

                //判断是否存在此支出
                if (roleSpendInfo.getBasicSpend().containsKey(repayName)) {

                    //删除此支出
                    roleSpendInfoHandler.removeBasicSpend(roleInfo, repayName);
            ...
            if (addNewDebt.containsKey(repayName)) {
                ...
                //删除此负债
                roleDebtsInfoHandler.removeNewDebt(roleDebtsInfo, repayName);

                //判断是否存在此支出
                if (roleSpendInfo.getAddNewSpend().containsKey(repayName)) {

                    //删除此支出
                    roleSpendInfoHandler.removeNewSpend(roleInfo, repayName);
